 ...
 *
 * Created on February 9, 2023, 11:02 PM
 */
...
int main(int argc, char** argv) {
    ...
    for(int i = 0; i<13; i++){ //randomizing the face
        int random = rand()%13;
        string temp  = face[i];
        face[i] = face[random];
        face[random] = temp;
    }
    for(int i = 0; i<4; i++){ //randomizing the suit
        int random = rand()%4;
        string temp  = suit[i];
        suit[i] = suit[random];
        suit[random] = temp;
    }
    for(int card = 0;card<NCARDS;card++){ //printing out the random cards
        string strCard;
        strCard=face[card%13];
        strCard+="_";
        strCard+=suit[card%4];
        cout<<setw(14)<<strCard<<endl; //version 1, testing shuffling code
    }
    /*const int one = 1;
